...
with open('./result/pickle/x_imp.pkl','rb') as f:
	points = pkl.load(f)
with open('./result/pickle/kikuchi.pkl','rb') as f:
	points2 = pkl.load(f)
with open('./result/pickle/QP_2classDB.pkl','rb') as f:
	QP_x,QP_y = pkl.load(f)
...
#-- plot average --#
plt.scatter(x_A,y_A,color = 'black',marker='x')
plt.scatter(x_B,y_B,color = 'gray',marker='o')
...
#plt.plot(val_x,val_k,color='red',label='2class Gilbert Algorithm')
#plt.scatter(points[0][0],points[0][1],color='red')
#plt.scatter(points[1][0],points[1][1],color='red')
#--kikuchi--#
b = np.array([points2[0][0],points2[0][1]])
a = np.array([points2[1][0],points2[1][1]])
weight = a - b
DBS_b = (-1)*(np.dot(a,weight)+np.dot(b,weight))/2
DBS_kikuchi = np.append(weight,DBS_b)
val_k = []
for i in range(len(val_x)):
	val_k.append((-1)*(val_x[i]*DBS_kikuchi[0]+DBS_kikuchi[2])/DBS_kikuchi[1])
plt.plot(val_x,val_k,color='royalblue')
#plt.plot(QP_x,QP_y,color='green',label='Quadratic Problem')
        
plt.scatter(points2[0][0],points2[0][1],color='b')
plt.scatter(points2[1][0],points2[1][1],color='b')
...
#--save png--#
OUTPUT_PATH = './image/2-class_kikuchi_'+str(num)+'P_imp.png'
